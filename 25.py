...
class DeviceSelectionEnv:
    ...
    def step(self, action):
        ...
        for idx in selected_indices:
            device = self.mec_server.devices[idx] 
            # Equation 10 from the paper
            # لتحويل وحدة الطاقة إلى نطاق مناسب وتجنب مشاكل الدقة العددية، مما يجعل قيمة B_k أسهل في التعامل والتفسير
            # أفتحوا ريدمي أمنية
            B_k = (device.cpu_freq ** 2) * TAU * MU_BITS * G * ENERGY_SCALE
            # قانون رقم 11
            self.e_max_per_device[idx] = max(self.e_max_per_device[idx] - B_k, 0)
            # يشمل الأجهزة المختارة (التي تم تحديثها) وغير المختارة (التي لم تتغير قيمها)
        self.e_max_total = np.sum(self.e_max_per_device)
        ...
        done = reward < 0 or self.e_max_total <= 0
